...
def check_vertically(grid)
    
    product = 0
    line = 0
    i = 0
    until line == grid.length - 3
        until i == grid[0].length
            temp_product = grid[line][i] * grid[line + 1][i] * grid[line + 2][i] * grid[line + 3][i]
...
def check_horizontally(array)
    ...
    i = 0
    until i + 3 == array.length
        temp_product = array[i] * array[i + 1] * array[i + 2] * array[i + 3]
            if temp_product > product
                product = temp_product
            end
        i += 1
...
grid.each do |line|
    if check_horizontally(line) > horizontal_product
        horizontal_product = check_horizontally(line)
    end
end
